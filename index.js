      ...
      if ((s4dmessage.content) == String(prefix) + 'welcomedm') {
        if ((s4dmessage.member).permissions.has(Permissions.FLAGS.MANAGE_GUILD)) {
          if (join_on_off == 'ON') {
            ...
            s4dmessage.channel.send({embeds: [{
            color: String('#ff0000'),
            title: String('> Les messages de Bienvenue n\'ont pas encore été activé !'),
        ...
        description: String('écrivez `ON` pour l\'activer ou bien `OFF`  pour le désactiver . '),
         ...
         s4d.message = collected.first();
           if (join_on_off == 'ON') {
            s4dmessage.channel.send({embeds: [{
            color: String('#33ff33'),
            title: String('> le Système de Bienvenue a belle et bien été **activé** ! '),
            }]}).then(async (s4dreply) =>{
               await delay(Number(2)*1000);
              (s4dmessage.channel).bulkDelete((4|1));
            ...
            s4dmessage.channel.send({embeds: [{
            color: String('#ff0000'),
            title: String('> Le Système de Bienvenue a belle et bien été **désactivé** !'),
            }]}).then(async (s4dreply) =>{
               await delay(Number(2)*1000);
              (s4dmessage.channel).bulkDelete((4|1));
            ...
            s4dmessage.channel.send({embeds: [{
            color: String('#ff0000'),
            title: String('> Oups ! Un erreur s\'est produite ! Merci de faire attention à l\'orthographe et aux majuscule !'),
            }]}).then(async (s4dreply) =>{
               await delay(Number(2)*1000);
              (s4dmessage.channel).bulkDelete((4|1));
    ...
    s4d.client.on('messageCreate', async (s4dmessage) => {
      if ((s4dmessage.content) == String(prefix) + 'setprefix') {
        if ((s4dmessage.member).permissions.has(Permissions.FLAGS.MANAGE_GUILD)) {
          (s4dmessage.channel).send({embeds: [{
          color: String('#3366ff'),
          title: String('Quelle sera le nouveau préfix du bot ?'),
